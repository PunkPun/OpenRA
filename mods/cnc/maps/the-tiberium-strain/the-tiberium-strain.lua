...
--- Are all members of this group either dead or idle?
---@param actors actor[]
---@return boolean
local function AllDeadOrIdle(actors)
...
local function AddVillagerObjective()
...
local function ScheduleVillageSweepers()
	if Nod.HasNoRequiredUnits() or #Utils.Where(Villagers.GetActors(), IsWestVillager) == 0 then
		return
	end

	if #GDI.GetGroundAttackers() > 0 then
		Trigger.AfterDelay(DateTime.Seconds(1), ScheduleVillageSweepers)
		return
	end
...
local function GuardArea(guards, center, range)
	...
	Trigger.OnEnteredProximityTrigger(center, range, function(target, id)
		if activated or IsMobileNod(target) then
			return
		end
	...
	Utils.Do(guards, function(guard)
		...
		Trigger.OnIdle(guard, function()
			...
			if refreshed or not activated or not AllDeadOrIdle(guards) then
				return
			end
...
local function PrepareVillagers()
	Trigger.OnPlayerDiscovered(Villagers, function(_, discoverer)
		...
		Trigger.AfterDelay(DateTime.Seconds(6), AddVillagerObjective)
	end)
...
local function PrepareAreaGuards()
	...
	Utils.Do(teams, function(team)
		...
		GuardArea(team.actors, center, radius, Nod)
	end)
end

--- Prepare the houses' survival objective, and their ability to call for help.
...
--- Prepare the reveal and patrol for Hard's Mammoth Tank.
---@param tank actor
...
local function SpawnHardUnits()
	...
	Utils.Do(spawns, function(spawn)
		local actor = Actor.Create(spawn.type, true, { Owner = GDI, Location = spawn.cell, Facing = spawn.facing })
...
Tick = function()
	if Nod.HasNoRequiredUnits() and NodArrived then
		MarkNodDead()
	end
