...
namespace OpenRA.Mods.Common.Traits
{
	class ClusterContents
	{
		...
		public ClusterContents(bool empty)
		{
			ClusterId = 0;
			IsEmpty = true;
		}
	...
	{
		protected readonly CellLayer<ClusterContents> clusterLayer;
		...
		public ClusterManager(World world)
		{
			this.world = world;
			map = world.Map;
			clusterLayer = new CellLayer<ClusterContents>(world.Map);
			emptyCluster = new ClusterContents(true);
		...
		{
			if (!clusterLayer.Contains(p1) || !clusterLayer.Contains(p2))
				return false;

			Console.WriteLine(clusterLayer[p1].ClusterId + " " + clusterLayer[p2].ClusterId);
			if (clusterLayer[p1] == clusterLayer[p2])
				return true;

			// Even though p1 and p2 are in different domains, it's possible
			// that some dynamic terrain (i.e. bridges) may connect them.
			return clusterLayer[p1].TransientConnections.Any(con => con.Active && clusterLayer[p2].TransientConnections.Contains(con));
		}
		...
		public void AddFixedConnection(IEnumerable<CPos> connectedCells)
		{
			...
			{
				if (!clusterLayer[c].TransientConnections.Contains(connection))
					clusterLayer[c].TransientConnections.Add(connection);
			}
		}
		...
		protected void UpdateTransientConnections(IEnumerable<ClusterTransientConnection> connections, ClusterContents oldCluster, ClusterContents newCluster)
		{
			...
			connections.Where(x => x.ConnectedCells.Any(y => clusterLayer[y] == newCluster))
				.Do(x =>
				{
			...
			connections.Where(x => !x.ConnectedCells.Any(y => clusterLayer[y] == oldCluster))
				.Do(x => newCluster.TransientConnections.Remove(x));
		}

		protected void UpdateCell(CPos cell, ClusterContents newCluster)
		{
			if (clusterLayer[cell] == newCluster)
				return;

			var oldCluster = clusterLayer[cell];
		...
		protected void AssignCluster(ClusterContents oldCluster, ClusterContents newCluster)
		{
			...
			oldCluster.Cells.Do(x => clusterLayer[x] = newCluster);
		...
		{
			var addedCells = dirtyCells.Where(cell => ClusterCondition(cell) && clusterLayer[cell].IsEmpty);
			var removedCells = dirtyCells.Where(cell => !ClusterCondition(cell) && !clusterLayer[cell].IsEmpty);

			foreach (var cell in addedCells)
			{
				Console.WriteLine("Added " + cell.X + " " + cell.Y);
				...
				foreach (var n in neighbors)
				{
					if (foundCluster == null)
					{
						...
						foundCluster = clusterLayer[n];
						UpdateCell(cell, clusterLayer[n]);
					}
					else
					{
						// Assimilate nearby neighbors
						AssignCluster(clusterLayer[n], foundCluster);
					}
				}
				...
				if (foundCluster == null)
				{
					...
					clusters.Add(clusterLayer[cell]);
				}
			}
			...
			foreach (var cell in removedCells)
			{
				...
				if (clusterLayer[cell].Cells.Count > 1)
				{
					// Select all neighbors inside the map boundaries
					var neighbors = CVec.Directions.Select(d => d + cell)
						.Where(c => clusterLayer[cell].Cells.Contains(c) && !removedCells.Contains(c)).ToList();

					// We don't need to test for splitting if it's an edge of a domain
					if (neighbors.Count >= 2)
					{
						Console.WriteLine("let the splitting commence");
						RecalculateCluster(clusterLayer[cell], removedCells.ToList());
					}
				}
		...
		protected virtual ushort BuildDomains()
		{
			...
			while (toProcess.Count != 0)
			{
				...
				while (clusterQueue.Count != 0)
				{
					...
					if (currentTrue)
					{
						clusterLayer[n] = cluster;
						cluster.Cells.Add(n);
					}
					else
						clusterLayer[n] = emptyCluster;
