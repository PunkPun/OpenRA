...
namespace OpenRA.Mods.Common.Activities
{
	...
	public class LayMines : Activity
	{
		...
		public LayMines(Actor self, List<CPos> minefield = null)
		{
			...
			manager = self.TraitOrDefault<DockClientManager>();
		...
		public override bool Tick(Actor self)
		{
			...
			{
				if (manager != null && !manager.IsTraitDisabled && ammoPools.Any(p => p.Info.Name == minelayer.Info.AmmoPoolName && !p.HasAmmo))
				{
					QueueChild(new MoveToDock(self, manager, minelayer.Info.RearmDockType, manager.DockLineColor));
					return false;
				}
