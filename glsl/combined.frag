...
vec4 SamplePalettedBilinear(uint samplerIndex, vec2 coords, vec2 textureSize)
{
	...
	vec4 c1 = texture(Palette, vec2(dot(x1, vChannelMask), vPalette));
	vec4 c2 = texture(Palette, vec2(dot(x2, vChannelMask), vPalette));
	vec4 c3 = texture(Palette, vec2(dot(x3, vChannelMask), vPalette));
	vec4 c4 = texture(Palette, vec2(dot(x4, vChannelMask), vPalette));
...
void main()
{
	...
	if (!(AntialiasPixelsPerTexel > 0.0 && isPaletted))
	{
		...
		vec2 p = vec2(dot(x, vChannelMask), vPalette);
		if (isPaletted)
			c = texture(Palette, p);
		else if (isColor)
			c = vTexCoord;
	...
	if (isPaletted && vPalette > 0.0)
		c = ColorShift(c, vPalette);
